...
- 已有配置充分无需 setup；只选领域配置不强制生成跟踪器或其他未选配置。
- 已定选择不重复提问，重要未决选择仍交用户；写入遵守既有授权范围。
- 多上下文不依赖 workspace 清单；两种指令文件按实际消费与已有属主选择。
- publish 更新同一工作项而非重复创建；Spec 位置归注册表；发布权限独立核对。
- 报告包含实际检查和局限，配置写好不等于远程可用；不擅自安装工具、试建 Issue 或迁移历史。
...
- 配置入口明确按所选范围检查；已有配置充分可跳过 setup，领域配置不强制跟踪器。
- 差异说明复用已有选择与授权，只询问重要未决事项；重复运行和覆盖限制保留。
- 入口按实际消费关系选择，多上下文按领域边界选择，根级注册与记录共享规则保留。
- 三个发布模板均先定位已有工作项、核对发布权限，并保留独立 Spec 的注册属主。
- 完成条件明确核对工具与访问能力，如实报告限制，不授权安装工具或通过试写验证连接。
