...
# Agent Note：多上下文共用仓库级工件注册与 Agent Notes

## 问题

多上下文中按当前子项目工作目录定位文档与记录，会造成属主和引用随入口变化。
...
[注册规则](../../../../skills/user-invoked/setup-matt-pocock-skills/artifact-registration.md)、[注册样板](https://github.com/peakdong68/my-skills/blob/dc2212e2853bbc98cb187e134c7fb411cb55e361/test-skills/skills/user-invoked/setup-matt-pocock-skills/artifacts.md)、[初始化](../../../../skills/user-invoked/setup-matt-pocock-skills/SKILL.md)、[记录部署](../../../../skills/user-invoked/setup-matt-pocock-skills/decision-records.md)、[领域消费](../../../../skills/user-invoked/setup-matt-pocock-skills/domain.md)、[领域建模](../../../../skills/model-invoked/domain-modeling/SKILL.md)、[记录规则](../../README.md)。

## 验证

已交付上述规则。Standards 与 Spec 对八份 setup/domain 文件的实施审查无实质发现；子目录与根目录执行定位同一注册入口和记录集，上下文内容仍按各自属主分布，记录范围按用户选择隔离；记录 README 同步加入相同归属和分类边界，通用源与本仓库副本逐字节一致，三份既有脚本也保持一致且未修改。
...
同一仓库范围共用管理根入口和记录系统；多上下文内容仍按各自属主分布，记录范围按用户选择隔离。
