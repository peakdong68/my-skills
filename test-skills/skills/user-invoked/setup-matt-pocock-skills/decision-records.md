...
The root config maps selected context identifiers from `CONTEXT-MAP.md` to distinct directories under `.agents/notes/`. Each directory has its own lifecycle/category tree and generated index; the root index routes readers to them. The map owns domain boundaries, while Notes config owns record enrollment and paths. Categories remain independent. Read the relevant context index and the selected shared index, following explicit cross-context links as needed.
...
Existing unpartitioned records remain in their original lifecycle/category paths and in the root index. Adding mappings does not migrate them. Migration requires an explicit source/destination agreement and link repair.
