...
智能体技能（Agent Skills）是一组按开发阶段组织的工程工作流技能。每项技能都编码了资深工程师遵循的特定流程。此元技能帮助你发现并为当前任务应用正确的技能。并提示用户当前已完成的工作以及建议下一步工作（参考：开发生命周期技能序列）。
...
任务到达
    ...
    ├── 提交/分支操作？ ─────────→ git-workflow-and-versioning（Git 工作流与版本控制）
...
3. **多项技能可能同时适用。** 功能实现可能依次涉及 `idea-refine` → `spec-driven-development` → `planning-and-task-breakdown` → `incremental-implementation` → `test-driven-development` → `code-review-and-quality` → `shipping-and-launch`。
...
规划阶段
  3. planning-and-task-breakdown  → 拆成可验证的小任务（用户斜杠命令调用 /plan）
...
交付阶段
  10. git-workflow-and-versioning  → 原子提交、干净历史
  11. ci-cd-and-automation         → 自动化质量门
  12. deprecation-and-migration    → 安全移除旧系统/API/功能
  13. documentation-and-adrs       → 记录决策，不只是做了什么
  14. shipping-and-launch          → 并行审查 + 发布清单（用户斜杠命令调用 /ship）
```
...
| 交付 | git-workflow-and-versioning   | 原子提交，干净的历史记录                |
