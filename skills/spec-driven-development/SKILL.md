...
> "规格说明已撰写并提交至 `docs/spec/feature_<date>_<id>_<topic>/spec-design.md`。请审查，并在我们开始制定实现计划之前告诉我是否需要进行任何更改。"

等待用户的回复。如果他们请求更改，进行修改并重新运行规范自查（步骤 8）。仅在用户批准后继续。

**批准的下一步：** 建议运行 `spec-review` 技能进行独立规范审查（由独立子代理执行）。审查通过后，执行命令`/plan`或调用 `planning-and-task-breakdown` 技能创建详细的实现计划。**不要**直接跳到编写代码。
