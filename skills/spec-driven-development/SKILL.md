...
**★ 使用 `Agent` 工具分派独立子代理执行审查**（subagent_type: `spec-reviewer`）。子代理以全新视角执行三层校验（L1 结构完整性 / L2 需求一致性 / L3 边界鲁棒性），产出结构化审查报告。作者不得自查——独立审查才能发现盲点。
...
**批准的下一步：** 执行命令 `/planning` 创建详细的实现计划和任务清单。**不要**直接跳到编写代码。
